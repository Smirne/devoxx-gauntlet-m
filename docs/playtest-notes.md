...
| 9 | "Voxxy feels too fast, it's almost hard to control" | done | Frozen constants, and he unfroze them: the 23 Sep rescale put `SPEED_SCALE = 0.25` on every px/s quantity. Her top speed is 72.5 px/s = 5.8 m/s, asserted in `tests/frozen-constants.test.ts`. This row said "in flight" for two days after the rescale had shipped. |
| 10 | "it's really hard to stop a character pointing the light in the right direction" | done | Two fixes, both shipped. The coast came down with #9 (it was 2.6 m at the old speeds). The other half was that letting go **turned the robot round**: a wall bounce reverses the normal component, so the last frames of a coast were the rebound and every direction ended facing backwards from the stick that had been held. `stepAim` now holds the heading off the stick — unless the world is moving the robot, which is the same flag `worldMoved` publishes for Biggy's roll. |
| 11 | "droid is pushing Biggy just by coming close, with no contacts" | done | The other half of what he unfroze on 23 Sep: the radii were guessed, and are now measured off the rigs (`docs/scale-and-units.md`). Voxxy 4.75, Droid 6.25, Biggy 9 sim px. **The crowd and the props were not re-measured with them** — see the structural list. |
| 12 | "when biggy pushes the door there should be some kind of animation" | done | It used to cease to exist — `removeWall` in the same frame as the hit. The collider still goes on that frame, because the opening has been earned; what is left is a prop with a clock on it, `jamFall` running 0 → 1 over `JAM_FALL_TIME` while the renderer drops the leaf. His follow-up on it — *"we are still walking through the crashed door"* — is closed too: `surfaceY` reads the fallen leaf, so they walk **on** it. |
| 13 | "the animation in the chapter 1-2 passage is too fast and too dark, and I'd zoom more" | two of three done; **the third is his eye** | *Too fast* — done and held there by a test. Measured, the transition drove Droid at **263% of the fastest he can physically move**, which is a physics-realism bug and not a pacing note; `tests/cutscene-pace.test.ts` now plays every transition in the game and asserts no robot is ever moved faster than its own `max`. *Zoom more* — done: `CUT_FRAME` in `scene.ts` frames chapters 1 and 3's transitions at 430×315 sim px instead of the storey's own rect, tracking the middle of the group rather than whichever robot was last driven. *Too dark* — **not changed, and deliberately not changed on a hunch.** Chapter 1 is a dark building and the robots carry the light; it also had a whole light-and-readability pass on 24 Sep after this note. Re-shot on the V37 build (`scratchpad/shove/cut1.png`, driven through the real keypad) so he can judge the current one rather than the one he played: the corridor reads by its room signs and the three lamps, and everything past their reach is black. If that is still too dark it is a mood change, and it is his call whether chapter 1 gets lighter or only its transition does. |
...
| 16 | "those two are maybe too near to each other?" | **closed — keep as is** | Two clue spots. Michele, 25 Sep: *"keep as is"*. |
| 17 | "I put all three robots in the room... needed different tries before finding the number" | **closed — keep as is** | Chapter 1's four-digit clue puzzle: a digit shows only under the right two-robot light mix, and the four go into the fire-door keypad. His call was to try the new arc markers (note 7) first; on 25 Sep, *"keep as is"*. |
...
| --- | --- | --- |
| ~~Voxxy's jump~~ **built** | Michele's idea, scoped down by him to *"just for one quiz. And for jumping around for fun"*, then approved outright on 24 Sep: *"Voxxy jump: let's make it. I'd keep E, when no other action is available."* | Built. The sim owns the arc (`hopPhase`, `JUMP_AIR`, `JUMP_RISE_M` — derived against closed-form projectile motion, not eased), the rig reads a 0→1 phase, and `E` falls through to it only when the chapter has no use for the key. `tests/party-tricks.test.ts`. His `E` ruling settled the other half too: the jump and the tow grab are both the fallback. |
...
| ~~Robots do not stand on the ground floor's raised lobby or its stairs~~ **fixed** | agent, cutscene round | `groundRiseM(x)` existed for this, said the renderer read it, and nothing did — a robot on the lobby plate stood half a metre inside it and one on the main flight was swallowed, which is why chapter 3's transition walked into a staircase. The sim publishes the raised surfaces as `groundPlates()` and `src/sim/surface.ts`, `surfaceY` in `scene.ts` reads them, and the 25 Sep staircase turn depended on it working. `tests/surface.test.ts`, `tests/stairs-driven.test.ts`. |
...
| *"Toilets are just on the west side, but it's not an important details. Cover it up."* — and, 25 Sep, *"Cover the toilets."* | **Done.** Sealed rather than moved: the south face is one unbroken run with a pair of shut leaves drawn in it and the pictogram over them, and the two internal partitions are gone with the doorway. Nothing in any chapter happens in there, and a room nobody can walk into is a room whose position nobody can check. The beat he offered — *"Wow, no queue"* in chapter 2, an actual queue in chapter 3 — is still available and unbuilt. |
...
| *"some flickering lights (hard to show on screenshot)"* · and, 25 Sep, with the location: *"chapter 2/3 the passage between the reception zone and the main hall. There's a line that flicker when the robot is walking"* | **Done.** Z-fighting, and the location is what made it findable. The raised lobby plate started 6 px west of `LOBBY_X` while the threshold's top tread ended there — both top faces at **y = -5.0000 exactly**, a 0.48 m ribbon running the flight's whole 22.6 m, plus the concrete riser caps above and below it, which the lip covered completely. It sits still until a light crosses it, which is why it only showed *while a robot walked* and never survived a screenshot. The plate stops at `LOBBY_X` now: abutting is fine, overlapping is not. `tests/coplanar.test.ts` is the class rather than the instance, and it found a **second** one nobody had reported — the first floor's corridor carpet running straight over the main staircase's head, 12.46 m² at y = 0.0000, now cut. |
...
| *"Robots still go through the handrail in the chapter transiction"* | **Done.** `stairExitRoutes` walks the corridor, turns in at `TURN_X` clear of the balustrade's east end and goes WEST into the pocket along its own axis — the one side that is open — arriving in file, deepest first, with Biggy stopping at the mouth because 1.30 m clear against his 1.44 is the truth of the building. `tests/staircase-clear.test.ts` walks the legs against the floor's walls: the fault was invisible to every test in the repo, all of which asked where a robot may WALK and none where a cutscene sends one. |
| *"there might be also intermediate challenges (eg: open the door for clue 3 with Droid and biggy). They might need a clue too?"* | **Half open — the mechanism landed, the content did not.** `Task.hint` takes a list (`string | readonly string[]`) and chapter 1 uses one. What has not been designed is the intermediate challenge itself: the door-for-clue-3 beat he sketched, or another. That part is a design call. |
| *"There's a light on the crates, robot exit fully visible. Light (emergency light?) flickers and stops, robots light up -> transition to game"* | **Done.** Built as he described it: `src/sim/opening.ts` owns the last emergency fitting on the stretch, `emergencyAt(t)` gives its 1-or-0 over `FLICKER_TIME`, and it dies into the three lamps coming up. `tests/opening.test.ts` and `tests/intro-light.test.ts` hold the sequence. |
| *"We'll need to add music!"* | **Done 25 Sep.** Four synthesised scores, one per chapter, four bars each, in `src/render/music.ts` — no audio files, per `CLAUDE.md`. Each score is its own deck with its own fader, so a chapter change never plays two keys at once (notes are booked up to 1.9 s ahead and the first version overlapped). `N` mutes. `tests/music.test.ts`. |
| *"We need to add the CRAB SANDWICH somewhere. That's the most famous part of the infamous devoxx food."* | **Done.** On the sandwich counter's hall-facing edge in chapter 3 (`CRAB`, `CRAB_REACH`), under the sign, where a robot walking past finds it. |
...
| item | note |
| --- | --- |
| ~~`rig.ts`'s `weather()` brightens dark materials~~ **fixed** | It wrote vertex colour as a ratio against the material's own colour, so a rust tint on a near-black part divided a light colour by a dark base and "wear" came out as white flakes. Censused: Droid had 11 meshes and 53 vertices pinned at the old per-channel clamp, Biggy 3 and 18, Voxxy none — her wear only ever lands on orange and white, which is why nobody had seen it. `MAX_WEAR_LUM_GAIN` now caps the **result** and scales all three channels together, so the tint keeps its hue and only its brightness is held down, and `biggy.ts`'s local workaround is no longer load-bearing. |
| ~~`buildLights` is ~95% of a chapter-2 sim step~~ **culled** | 0.9 → 2.63 ms per cast across two changes (colliders, and the skirt doubling the light count). The note said the next change to that path should range-cull first; the collider sweep was that change, and `castPoly` now filters occluders to the lamp's own reach before casting. It cannot change a polygon — every ray is clipped at `range`, and a rect outside the square of half-width `range` about the lamp is further away than `range` in the max norm and so in Euclid too. |
| ~~Four helper sets want promoting to `rig.ts`~~ **promoted** | Three agents had independently written their own: a UV-carrying patch, a clearcoat material, a surface kit, a bellows. `rig.ts` now owns `spherePatch`, `ovalPatch`, `revolvePatch`, `latheProfile`, `puck`, `dotGrid`, `roundedBox`, `panelMaterial` and `glowMaterial`. The rivet aim went with them: `boltRing` takes `aimSlope` (`dr/dy`, exact for any surface of revolution) instead of pointing at a sphere's centre, which had put Biggy's dome line 12° off on all eighteen. |
| ~~`gaitSpeed`'s `tanh` compression~~ **removed** | The surviving cousin of the deleted `HUD_PX_PER_MPS`. Measured before removing it, and it was not doing the job it existed for: at Voxxy's top speed the cadence, the arm rate and the knee angle were identical with it and without, because `MAX_STEP_FREQ` binds first either way. What it WAS doing is breaking the file's own promise that a planted foot travels backward at the body's speed — her foot ran at 2.69 m/s while she moved at 5.80, so she skated forward at 3.1 m/s, and all three did it. Gone; foot travel now equals body speed on all three to the last digit. |
| `CUT_WALK_SPEED` is dead | The cutscene rewrite made walks duration-driven. Still exported, scaled and asserted by `tests/frozen-constants.test.ts`; nothing reads it. **Re-checked 25 Sep, still true.** Left alone on purpose: it is a frozen constant, and deleting one is a human decision even when the thing it froze has no callers. |
| Crowd and prop radii are the generous ones now | A conference-goer is 0.8-1.28 m wide and the cake crate is 1.36 m, sized when the robots were twice their current width. **Re-checked 25 Sep, still true**: every visitor, queue person and named extra in chapter 3 is `r: 8` — 1.28 m across, against Biggy's 1.44 m. It is the same guess-versus-measure that note 11 was, one object class along, and narrowing them changes how the catering gate and the hall read, so it is his call rather than a builder's. |
| ~~`pushBiggy` and `stepTow` ignore Biggy's mass~~ **closed by him** | Both add `force · dt` straight to his velocity without dividing by mass; only `botsCollide` reads it. Making them mass-aware retunes chapter 2's roller door, which is frozen physics. Michele, 25 Sep: *"fine as is I'd say."* |
| ~~A careful player may never see the OutOfMemoryError~~ **closed by him** | The HUD reads `heap 4/5`, which is what makes the beat survivable and also what makes it skippable; the guaranteed version was a seventh crate. Michele, 25 Sep: *"I was seeing with 5 crates, that's fine."* No change. |
...
| ~~`E` precedence at the Sticker Mine~~ **it was a soft-lock; fixed** | This row said the minigames' key handler runs first and that the sticker's booth "shares the grid with the keynote speaker's hiding places". Audited 25 Sep, and it was worse than the row made it sound: the sticker's spot and the speaker's are both `inFrontOf(booth)`, so when the chapter's RNG picked the Sticker Mine **they were the same point** — Voxxy's `E` at the speaker got the sticker's refusal, which consumes the key, and chapter 3 cannot be finished without the speaker. Measured over 200 seeds: **35, one run in six.** The precedence itself stays (a refusal that names a reason keeps the key, by his own rule); what changed is that the speaker no longer hides inside a minigame's key circle — `Minigames.keySpots()` publishes them and `SPEAKER_CLEAR` keeps the spine out. The two tests that were clearing the sticker first now run the errand with it still live, which is how a workaround becomes a test. |
...
| `tools/progress/shots/` is 50+ MB | Prune before submission. **53 MB on 25 Sep.** Not pruned by the agent: they are his build history and deleting them is not a builder's call. |
...
| --- | --- | --- |
| *"The door opens through Voxxy. What about vertical opening? shutter door?"* | **Done — arc push, his choice.** For the one second of `FIRE_SWING_TIME` the `firedoor` collider has gone and the `fireleaf` walls are not in yet, so a robot in the arc was passed straight through — and that robot is Voxxy, because the keypad she has just used is on the door. `sweepFireDoor` finds the closest point on each leaf's centre line, puts the robot out along the leaf's own normal and gives it the leaf's speed **at that radius**: a point `r` along a leaf turning at `w` travels at `w r`, so a robot near the tip is thrown harder than one near the hinge, out of the arithmetic rather than out of a special case. Measured at first contact: 31.3 px/s against the leaf edge's 31.26. `tests/fire-door.test.ts`. |
